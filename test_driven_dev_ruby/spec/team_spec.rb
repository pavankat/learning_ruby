...
describe Team do
	...
	it "is favored if it has a celebrity in it" do 
		Team.new("Random name", [ "George Clooney", "Jose Mota" ]).should be_favored
	end
	
	it "complains if there is a bad word in the name" do
		expect { Team.new("Crappy name") }.to \
		raise_error
	end
	...
	context "given a bad list of players" do
		...
		it "fails to create given a bad player list" do
			expect { Team.new("Random name", bad_players) }.to \
			raise_error
		end
